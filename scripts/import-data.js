...
async function importarPartidos() {
  ...
  for (const item of data) {
    const partido = {
      ...
      color: item.color,
      logo_url: item.logo_bloque ? `https://www.congreso.gob.gt/uploads/bloques/${item.logo_bloque}` : null,
...
async function importarDiputados(partidosMap) {
  ...
  for (const item of data) {
    ...
    const diputado = {
      ...
      foto_url: item.foto_perfil ? `https://www.congreso.gob.gt/uploads/diputados/${item.foto_perfil}` : null,
      ...
      cv_url: item.cv ? `https://www.congreso.gob.gt/uploads/diputados/${item.cv}` : null,
